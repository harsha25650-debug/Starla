...
def get_prefix(bot, message):
    ...
    # 1. ROOT OVERRIDE: Owner always bypasses prefix constraints
    is_root_owner = False
    ...
    # 2. Check Premium Persistent Database Status
    ...
    # 3. Apply Empty Token Prefix mapping if authorized
    ...
    # 4. Standard guild routing prefix fallback map
    ...
    async def on_message(self, message):
        ...
        if bot_mentioned:
            ...
            if "harsh" in content:
                ...
                await message.reply(f"{v} Greetings. I am **Starla**, optimized by **Harsh**. Your configuration prefix is `{current_prefix}`. Use `{current_prefix}help` to access service modules. {heart} {mor}")
                return
...
# --- 💎 PREMIUM SYSTEM PERSISTENT STORAGE COG ---
class PremiumSystem(commands.Cog):
    ...
    async def list_users(self, ctx):
        ...
        msg += f"👑 **[ROOT AUTHORIZED]** (Global Auto-Bypass) • **{ctx.author.name}**\n"
