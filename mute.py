...
class Mute(commands.Cog):
    ...
    async def get_next_case(self):
        if not os.path.exists(self.db_file): return 1
        with open(self.db_file, "r") as f:
            data = json.load(f)
        return data.get("case_count", 0) + 1

    @commands.hybrid_command(name="mute", description="Mute a member (Permanent by default)")
    @app_commands.describe(member="Member to mute", duration="Optional: 10m, 1h, etc. Leave empty for Permanent", reason="Reason for mute")
    ...
    async def mute(self, ctx, member: discord.Member, duration: str = None, *, reason: str = "No reason provided"):
        ...
        # Default Logic: Agar duration nahi di, toh "Permanent"
        ...
        # Auto-Unmute logic sirf tab chalega jab duration di ho
        if seconds:
            await asyncio.sleep(seconds)
            if muted_role in member.roles:
                await member.remove_roles(muted_role)
                print(f"Auto-unmuted {member.name} after {duration}")
